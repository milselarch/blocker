...
const RegexColorizer = (function () {
  ...
  *------------------------------------ */
  ...
  error.contains = function (value) {
    // console.log('asdasd', Object.values(error))
    return Object.values(error).indexOf(value) > -1
  }
  ...
  *------------------------------------ */
  ...
  class HighlightArray extends Array {
  ...
  function ErrorGroup (str, desc) {
    const self = this
    self.desc = desc
    self.str = str

    self.toHTML = () => {
      // console.log("ERR", error.values(), desc)
      assert(error.contains(self.desc))
      return '<b class="err"' + (
        self.desc ? ' title="' + self.desc + '"' : ''
      ) + '>' + self.str + '</b>'
    }
  }
  ...
  function CharClass (chars) {
    this.toHTML = () => {
      return '<i>' + parseCharClass(chars) + '</i>'
    }
  }

  function Group (content, depth) {
    const self = this
    ...
    self.toHTML = () => {
      ...
      if (typeof self.content === 'string') {
        // console.log('CONTENT-IS_STRING')
        rawContent = self.content
      } else {
        rawContent = self.content
        rawContent = rawContent.toHTML()
      }

      // console.log('RAWCOTNET', [rawContent])
      return '<b class="g' + depth + '">' + rawContent + '</b>'
    }
  ...
  function MetaSequence (m) {
    this.toHTML = () => {
      return '<b>' + m + '</b>'
    }
  }

  function BackReference (content, trailing) {
    this.toHTML = () => {
      return '<b>\\' + content + '</b>' + trailing
    }
  }

  function EscapedLiteral (m) {
    this.toHTML = () => {
      return expandHtmlEntities(m)
    }
  }

  function Quantifier (style, m) {
    this.toHTML = () => {
      return (style ? '<b class="' + style + '">' : '<b>') + m + '</b>'
    }
  }
 ...
 */
  self.colorizeText = function (pattern) {
    ...
    const outputs = new HighlightArray()
    ...
    while (match = regexToken.exec(pattern)) {
      ...
      if (char0 === '[') {
        ...
        if (m.length === 2) { // m is "(?"
          outputs.push(new ErrorGroup(m, error.INVALID_GROUP_TYPE))
          ...
          const group = new Group(m, groupStyleDepth)
          openGroups.push({
        ...
        if (!openGroups.length) {
          outputs.push(new ErrorGroup(')', error.UNBALANCED_RIGHT_PAREN))
          output += errorize(')', error.UNBALANCED_RIGHT_PAREN)
          lastToken = {quantifiable: false}
        } else {
          outputs.push(new Group(')', groupStyleDepth))
        ...
        if (/^[1-9]/.test(char1)) {
          ...
          if (num > 0) {
            outputs.push(new BackReference(num, nonBackrefDigits))
            output += '<b>\\' + num + '</b>' + nonBackrefDigits
          } else {
            var parts = /^\\([0-3][0-7]{0,2}|[4-7][0-7]?|[89])([0-9]*)/.exec(m)
            outputs.push(new BackReference(parts[1], parts[2]))
          ...
          /* Browsers differ on how they handle:
                      ...
                      */
          if (/^\\[cux]$/.test(m)) {
            outputs.push(m, error.INCOMPLETE_TOKEN)
            output += errorize(m, error.INCOMPLETE_TOKEN)
            lastToken = {quantifiable: false}
            // Unquantifiable metasequence
          } else if ('bB'.indexOf(char1) > -1) {
            outputs.push(new MetaSequence(m))
            output += '<b>' + m + '</b>'
            lastToken = {quantifiable: false}
          ...
          outputs.push(new ErrorGroup(error.INCOMPLETE_TOKEN))
          output += errorize(m, error.INCOMPLETE_TOKEN)
          // Don't need to set lastToken since this is the end of the line
          // Escaped literal character
        } else {
          outputs.push(new EscapedLiteral(m))
          output += expandHtmlEntities(m)
          lastToken = {quantifiable: true}
        }
        // Quantifier
        ...
        if (lastToken.quantifiable) {
          ...
          if (interval && (+interval[1] > 65535 || (interval[2] && +interval[2] > 65535))) {
            outputs.push(new ErrorGroup(m, error.INTERVAL_OVERFLOW))
        ...
        if (lastToken.type === type.NONE || (lastToken.type === type.ALTERNATOR && !openGroups.length)) {
          outputs.push(new ErrorGroup(m, error.IMPROPER_EMPTY_ALTERNATIVE))
          ...
          if (openGroups.length) {
            ...
            outputs.push(new BoldHighlight('|'))
            output += '<b>|</b>'
          }
        }
        ...
        outputs.push(new BoldHighlight(m))
        output += '<b>' + m + '</b>'
        lastToken = {quantifiable: false}
        // Dot (.)
      } else if (m === '.') {
    ...
    openGroups.map(openGroup => {
      // nIndex refers to index from from end of output
      // nIndex = -1 means group starts one chars from end
      // nIndex = -2 means group starts two chars from end
      // etc etc
      const errGroup = new ErrorGroup(
        openGroup.opening, error.UNBALANCED_LEFT_PAREN
      )

      openGroup.nIndex = openGroup.index - output.length
      const groupIndex = outputs.indexOf(openGroup.group)
      outputs[groupIndex].setContent(errGroup)
      // console.log('ERR', outputs[groupIndex].content, outputs[groupIndex].toHTML())
      return openGroup
    })

    let numCharsAdded = 0, errorIndex, i
    ...
    // console.log("OUTPUTZ", outputs.toHTML())
    // console.log("OUTPUTS", outputs.map(o => o.toHTML()))
    return output
  }
...
if (require.main === module) {
  console.log('POTATO')
  const out = RegexColorizer.colorizeText('www.youtub((e.com/watch*')
  console.log(out)
}
